...
export function subMenu_(subMenu) {
  ...
  if (subMenu && subMenu.length > 0) {
    for (let i = 0; i < subMenu.length; i++) {
      const element = {}
      element.name = subMenu[i].menuName
      element.path = subMenu[i].menuPath
      element.redirect = subMenu[i].menuMedirect
      element.meta = subMenu[i].menuMeta

      // 处理组件
      if (subMenu[i].menuComponent === 'Layout') {
        element.component = Layout
      } else {
        const component = subMenu[i].menuComponent
        element.component = (resolve) => require([`@/views/${component}`], resolve)
      }
      // 子路由
      element.children = subMenu_(subMenu[i].children)
      arr.push(element)
    }
  }
  return arr
